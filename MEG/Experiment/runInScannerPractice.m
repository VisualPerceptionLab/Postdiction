...
try
    ...
    % Ask about grating contrast
%     gratingContrast = input('Grating contrast? (0-1): ');
    noiseContrast = 0.2;
    
    % Ask about proportion of no-grating trials
%     propOmission = input('PO? (0-1): ');
    
    if propOmission > 0.75
        disp('propOmission > 0.75. Counterbalancing may fail.');
        continueExp = input('Are you sure you want to continue?: (1: yes, 2: abort): ');
        if continueExp == 2
...
%     getStaircases(subjID,resultDir,runType);
    ...
    textSize = round(30*(heightRatioDifference));
