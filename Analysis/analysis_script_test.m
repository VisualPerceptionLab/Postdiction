...
% between visual (given condition)
presentationTimesAV=results{1}.presentationTime(find(results{1}.condition==1),:);
nBlocks = 1;
nTrials = 16;
for iBlock = 1:nBlocks
    presentationTimesAV(:,2) - presentationTimesAV(:,1)
    presentationTimesAV(:,3) - presentationTimesAV(:,2)
    presentationTimesAV(:,4) - presentationTimesAV(:,3)
end
