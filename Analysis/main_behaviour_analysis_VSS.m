...
%         text(1.5, 0.85, '*', 'FontSize', 18, 'HorizontalAlignment', 'center');
        ...
        x_labels = {"No Flash", "Illusion", "Real Flash"};
        xticklabels(x_labels)
        % Customize subplot title
        title(["Group average reported flashes"]);%: ", index]);
        xlabel('Conditions');
        ylabel('3 flash');
        ylim([0 1])
        figure;
        ...
        [h, p_ver3_AV_3flash, ~, s]= ttest(squeeze(group_data_conf(1,4,:)), squeeze(group_data_conf(2,2,:)), tail='right')
        ...
        % Customize subplot title
        title(["Group average confidence"]);
...
%         
        ...
        % Add a line to denote significance comparison
        x = [0.85 1.85]; % X-coordinates of the two bars being compared
        y = [3.88 3.88]; % Y-coordinate (above the taller bar)
        plot(x, y, '-k', 'LineWidth', 1.5); % Line connecting the bars (black)
        % Add a significance annotation (assuming the difference is significant)
        text(mean(x), y(1) + 0.01, '**', 'FontSize', 18, 'HorizontalAlignment', 'center');
        ...
        %p_ver2_3flash = 0.0161
        % Add a line to denote significance comparison
%         x = [2.15 3.15]; % X-coordinates of the two bars being compared
%         y = [3.86 3.86]; % Y-coordinate (above the taller bar)
%         plot(x, y, '-k', 'LineWidth', 1.5); % Line connecting the bars (black)
%         % Add a significance annotation (assuming the difference is significant)
%         text(mean(x), y(1) + 0.01, '*', 'FontSize', 18, 'HorizontalAlignment', 'center');
        ...
        xticklabels(x_labels);
        errorbar(x_err(:,[1 2]), conf_data([1,2,4],:), plotste([1,2,4],:), '.', 'LineWidth', 1.5);
        legend({'2 flash', '3 flash', '', ''}, 'Location', 'northeastoutside');
        hold off;
