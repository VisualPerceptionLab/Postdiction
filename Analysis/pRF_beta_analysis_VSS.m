...
%     Roi{6,4} = Mainvols{4}(R_mask > 0.5 & Varvol > varexpmin & Sigvol < Sigmax & Sigvol > Sigmin & ~isnan(Mainvols{4}) & ~ROI1_coord_ind & ~ROI2_coord_ind & ~ROI3_coord_ind);
    ...
    AV_betas = Roi{2,2};
    ver2_betas = Roi{2,1};
    AV_x = Xvol(ROI2_coord_ind & ~isnan(Mainvols{2}));
    ver2_x = Xvol(ROI2_coord_ind & ~isnan(Mainvols{1}));
    AV_sig = Sigvol(ROI2_coord_ind & ~isnan(Mainvols{2}));
    ver2_sig = Sigvol(ROI2_coord_ind & ~isnan(Mainvols{1}));
    AV_ecc_corr = corr(AV_betas,abs(AV_sig));
    ver2_ecc_corr = corr(ver2_betas, abs(ver2_sig));
    
    ecc_hyp_results(iSubj,:) = [AV_ecc_corr, ver2_ecc_corr];
...
%     end
    ...
    %Roi{5,5} = Mainvols{5}(Xvol>= bullseye.xmin - Sigvol & Xvol<= bullseye.xmax + Sigvol & Yvol >= bullseye.ymin - Sigvol & Yvol <= bullseye.ymax + Sigvol & Varvol > varexpmin & Sigvol < Sigmax & Sigvol > Sigmin);
...
[h,p,t]=ttest(ecc_hyp_results(:,2),ecc_hyp_results(:,1), 'tail','right')
%%
if ~withinsubject
        % Create a barplot for each condition, where the betas per ROI are
    % displayed.
...
%         ploterrors(:,cond) = {ROI1_ste, ROI2_ste, ROI3_ste, ROI4_ste, ROI5_ste};
    ...
    cond2ROI2 =group_data_means(:,3,1);
...
%     ROI2_cond5_betas = group_data_means(:,2,5)
    [h,p_ill,ci,stats] = ttest(cond2ROI2, cond1ROI2, Tail='right')
...
% hold off;
    conditions = 3;
    % Create a subplot in the 4x4 grid
    plotmeans=plotmeans([1 2 3 4],:)
    plotmeans= plotmeans(:,[1 2 4]);
    ploterrors=ploterrors([1 2 3 4],:)
    ...
    hold on;
    for cond = 1:conditions
        x = (1:ROI_nr) - .5 + cond*0.25;  % Adjust x-coordinates for each set of bars
    ...
    title(['Neural activation per location']);%, num2str(length(prfSubjects)), ' subjects']); % betatext, prf text, [', ']
    ylim([0 14])
%     xlabel('rROIs');
    ylabel('Beta');
...
%     xticklabels(x_labels)
    ...
    sigPairs = {{1.75,2.15}, {1.75,1.95}};%, {3,4}}
...
%     [h,p,t] = ttest(ill_quot, ver2_quot, 'tail','right')
%     [h,p,t] = ttest(ver3_quot-ill_quot, ver3_quot-ver2_quot, 'tail','right')
