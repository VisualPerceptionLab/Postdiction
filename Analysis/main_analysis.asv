...
for iSubject=1:nSubjects
    ...
    if files_found(1)>0
        scanned_pp  = scanned_pp +1;
        scanned_list = [scanned_list; subject]
        behaviour_data{scanned_pp} = analyse_behaviour(subject, results_path, scanner);
    end
end
...
if group_analysis == true
    figure;
        ...
        bar(group_data_mean, 'LineWidth', 2);
    ...
    %         x = (1:2) - .48 + cond*0.19;  % Adjust x-coordinates for each set of bars
        errorbar(x, group_data_mean, group_data_ste, '.', 'LineWidth', 1.5);
...
%         text(1.5, 0.85, '*', 'FontSize', 18, 'HorizontalAlignment', 'center');
        ...
        x_labels = {"congr2", "rabbit", "mask", "congr3"};
        xticklabels(x_labels)
        % Customize subplot title
        title(["Group average for subjects: ", scanned_pp]);
        xlabel('Conditions');
        ylabel('3 flash');
        ylim([0 1])
        figure;
...
%         text(mean(x), y(1) + 0.01, '*', 'FontSize', 18, 'HorizontalAlignment', 'center');
        ...
        xticklabels(x_labels);
        errorbar(x_err, conf_data, plotste, '.', 'LineWidth', 1.5);
...
for iSubject=1:scanned_pp
    subject_behaviour = behaviour_data{iSubject};
    subplot(4, ceil(scanned_pp/4), iSubject);
...
%     conf_data(:,2) = 4-(nanmean(undesired, 2)-1);
    AV_confs_block = [];
    for iRun=1:length(subject_behaviour)
        AV_confs_block = [AV_confs_block subject_behaviour{iRun}.confidences.desired.cond2.all{:}];
    end
    % Convert ans to measures
    AV_confs = 5-(AV_confs_block(AV_confs_block>0)-10);
    histogram(AV_confs);%, 'LineWidth', 2);
