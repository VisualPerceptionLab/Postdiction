...
try
    ...
    % Ask about tone-shape contingency
    toneOrientation = input('Cue map? (1 or 2): ');
    % 1: Falling tones predict left-tilted (135 deg) grating, rising
    % tones predict right-tilted (45 deg) grating.
    % 2: Falling tones predict right-tilted (45 deg) grating, rising
    % tones predict left-tilted (135 deg) grating.
    
    gratingContrast = 0.1;
    noiseContrast = 0.2;
    
    % Ask about proportion of no-grating trials
    propOmission.prob = 0;
    ...
    if showInstructions == 1
        % two quick practice blocks
        nBlocks = 2 %2
        nTrialsPerBlock = 16 %64 (should be multiple of 32)
    elseif showInstructions == 2
        % two regular sized practice blocks
        nBlocks = 2 %2
        nTrialsPerBlock = 32 %64 (should be multiple of 32)
    end
    ...
    resultFile = sprintf('results_practice_%d_%d_%d_%d_%d_%d.mat',currentTime);
...
%     wrapat = round(100/widthRatioDifference); 
    wrapat = 100; 
    ...
    if showInstructions == 1
        showInstructions_MainExpt(toneOrientation,background,Lmin,volume,wrapat,vspacing);
    end
    ...
    for iBlock = 1:nBlocks
        ...
        if propOmission.prob < 1
            while trialSequence(1,1) == 0 || ~isempty(strfind(trialSequence(:,1)',[0 0 0 0 0 0]))
                disp('reshuffle trials')
                trialSequence = trialStructure(randperm(nTrialsPerBlock),:);
            end
        end
        ...
        data{iBlock} = oneBlock(gratingContrast, noiseContrast, nTrialsPerBlock, time, toneOrientation, trialSequence, background, Lmin, volume, mriTiming, feedback, 0);
